            ...
            <div class="navbar-buttons mbr-section-btn">
                ...
                <?php if (!$talkingToDriver): ?>
                    <a class="btn btn-sm btn-primary display-4" href="#<?php echo __d('mobirise/default', 'solicitar')?>">
                        ...
                        <?php echo __d('mobirise/default', __d('mobirise/driver_profile', 'Contactar a %s', $profile['DriverProfile']['driver_name']))?>
                    </a>
                <?php else:?>
                    <?php echo $this->Html->link('<span class="mbri-cust-feedback mbr-iconfont mbr-iconfont-btn"></span> '.__d('mobirise/driver_profile', 'Ver mis mensajes con %s', Driver::shortenName($profile['DriverProfile']['driver_name'])), array('controller'=>'conversations', 'action'=>'messages', $talkingToDriver), array('escape'=>false, 'class'=>'btn btn-sm btn-primary display-4'))?>
                <?php endif ?>
            </div>
