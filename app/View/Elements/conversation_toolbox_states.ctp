    ...
    <div class="input-group">
        <span class="input-group-btn">
            <button type="button" class="btn btn-<?php echo $btnType?> dropleft-toggle" data-toggle="dropdown">
                ...
                elseif ($data['TravelConversationMeta']['state'] == DriverTravelerConversation::$STATE_TRAVEL_DONE) echo '<i class="fa fa-thumbs-up"></i> Realizado';
                elseif ($data['TravelConversationMeta']['state'] == DriverTravelerConversation::$STATE_TRAVEL_PAID) echo '<i class="fa fa-dollar"></i> Pagado';
                ?>
                <span class="caret"></span>
            </button>
            <div class="dropdown-menu">
                <?php if($data['TravelConversationMeta']['state'] != DriverTravelerConversation::$STATE_NONE):?>
                    <div><?php echo $this->Form->button('Ninguno', array('class'=>'btn btn-default states-btn', 'data-url' => $this->Html->url(array('action' => 'set_state', $data['DriverTravel']['id'], DriverTravelerConversation::$STATE_NONE), true)), true);?></div>
                <?php endif?>

                <?php if($data['TravelConversationMeta']['state'] != DriverTravelerConversation::$STATE_TRAVEL_DONE):?>
                    <div><?php echo $this->Form->button('<i class="fa fa-thumbs-up"></i> Realizado', array('class'=>'btn btn-warning states-btn', 'data-url' => $this->Html->url(array('action' => 'set_state', $data['DriverTravel']['id'], DriverTravelerConversation::$STATE_TRAVEL_DONE), true), 'escape'=>false), true);?></div>
                <?php endif?>

                <?php if($data['TravelConversationMeta']['state'] != DriverTravelerConversation::$STATE_TRAVEL_PAID):?>
                    <div><?php echo $this->Form->button('<i class="fa fa-dollar"></i> Pagado', array('class'=>'btn btn-success states-btn', 'data-url' => $this->Html->url(array('action' => 'set_state', $data['DriverTravel']['id'], DriverTravelerConversation::$STATE_TRAVEL_PAID), true), 'escape'=>false), true);?></div>
                <?php endif?>
            </div>
        </span>
